...
module aptos_monkeys_rafflor::pseudorandom {
    use std::bcs;
    use std::error;
    use std::hash;
    use std::signer;
    ...
    public entry fun init(root: &signer) {
        ...
        assert!(signer::address_of(root) == @aptos_monkeys_rafflor, error::permission_denied(ENOT_ROOT));
        if (!exists<Counter>(signer::address_of(root))) {
    ...
    fun increment(): u64 acquires Counter {
        let c_ref = &mut borrow_global_mut<Counter>(@aptos_monkeys_rafflor).value;
        *c_ref = *c_ref + 1;
        *c_ref
    }
